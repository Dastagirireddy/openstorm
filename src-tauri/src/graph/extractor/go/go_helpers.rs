...
impl GoExtractor {
    pub(super) fn extract_function(
        ...
        nodes.push(create_node(NodeKind::Function, &name, file_path, start, end, "go"));
    ...
    pub(super) fn extract_type(
        ...
        let start = node.start_position().row as u32 + 1;
        let end = node.end_position().row as u32 + 1;
        nodes.push(create_node(NodeKind::Struct, &name, file_path, start, end, "go"));
    }
    ...
    pub(super) fn extract_import(
        ...
        let name = import_path.replace("import ", "").replace("(", "").replace(")", "").trim().to_string();

        let file_name = file_path.rsplit('/').next().unwrap_or(file_path);
        let source = format!("{}::{}", file_path, file_name);

        nodes.push(create_node(NodeKind::Import, &name, file_path, line, line, "go"));
