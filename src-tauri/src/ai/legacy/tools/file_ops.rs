...
use super::super::agent::FileModification;

/// Format a FileModification as unified diff text for tool result strings.
fn format_diff_as_text(modification: &FileModification) -> String {
    let mut out = String::new();
    let mut context_count = 0u32;
    let mut hunk_start = 0usize;

    for (i, line) in modification.diff.iter().enumerate() {
        match line.line_type.as_str() {
            "add" => {
                if context_count > 0 || i == 0 {
                    hunk_start = i;
                    context_count = 0;
                }
                out.push_str(&format!("+{}\n", line.content));
            ...
            "context" => {
                ...
                if context_count <= 3 || (i + 1 < modification.diff.len() &&
                    matches!(modification.diff[i + 1].line_type.as_str(), "add" | "delete")) {
                    out.push_str(&format!(" {}\n", line.content));
                } else if context_count == 4 {
                    out.push_str(" ...\n");
                }
            }
            _ => {}
    ...
    if out.is_empty() {
        format!("{} lines added, {} lines removed", modification.lines_added, modification.lines_removed)
    } else {
        let header = format!("@@ {} lines added, {} lines removed @@\n", modification.lines_added, modification.lines_removed);
        format!("{}{}", header, out)
    }
}
        ...
        match fs::write(&full_path, content).await {
            Ok(_) => {
                ...
                if old_content != content {
                    ...
                    let diff_text = format_diff_as_text(&modification);
                    let mut pending = self.pending_file_modifications.lock().unwrap();
                    pending.push(modification);
                    format!("Successfully wrote to {}\n{}", path, diff_text)
                } else {
                    format!("Successfully wrote to {} (no changes)", path)
                }
            }
        ...
        match fs::write(&full_path, &result).await {
            Ok(_) => {
                ...
                if content != result {
                    ...
                    format!("Successfully edited {} (no changes)", path)
                }
            }
            Err(e) => format!("Error writing file: {}", e),
        }
