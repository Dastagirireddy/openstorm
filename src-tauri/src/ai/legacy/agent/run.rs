...
use super::Agent;
use crate::ai::legacy::context::{ContextManager, AiSessionLog};
use crate::ai::legacy::permissions::PermissionResult;
use crate::ai::legacy::provider::*;
...
impl Agent {
    ...
    pub fn embedding_store(&self) -> Arc<tokio::sync::Mutex<crate::ai::legacy::embedding_store::EmbeddingStore>> {
        self.embedding_store.clone()
    }

    /// Get the cost tracker for external access.
    pub fn cost_tracker(&self) -> crate::ai::legacy::cost_tracker::SharedCostTracker {
    ...
    async fn run_inner(
        ...
        let mut denied_call_counts: HashMap<String, u32> = HashMap::new();
    ...
    async fn handle_response(
        ...
        denied_call_counts: &mut HashMap<String, u32>,
        tx: &mpsc::Sender<AgentEvent>,
    ) -> Result<(), ProviderError> {
            ...
            match &perm_result {
                PermissionResult::Denied { reason } => {
                    *consecutive_failures += 1;
                    // Track denied call to prevent retry loops
                    let dedup_key = format!("{}:{}", call.function.name, call.function.arguments);
                    let count = denied_call_counts.entry(dedup_key).or_insert(0);
                    *count += 1;
                    let result = if *count >= 2 {
                        format!(
                            "Tool '{}' denied: {}. This tool has been denied {} times now. STOP retrying it. Use a completely different approach or tell the user you cannot complete this task.",
                            call.function.name, reason, count
                        )
                    } else {
                        format!(
                ...
                PermissionResult::ApprovalRequired { .. } => {
                    let preview = crate::ai::legacy::agent::preview::generate_tool_preview(self, &call.function.name, &call.function.arguments);
                    let _ = tx
                    ...
                    if !approved {
                        ...
                        *count += 1;
                        let result = if *count >= 2 {
                            format!(
                                "Tool execution denied by user. This tool has been denied {} times now. STOP retrying it. Use a completely different approach or tell the user you cannot complete this task.",
                                count
                            )
                        } else {
