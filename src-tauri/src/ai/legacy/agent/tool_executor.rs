...
use crate::ai::legacy::provider::Message;
...
pub async fn execute_tool_call(
    agent: &Agent,
    call: &crate::ai::legacy::provider::ToolCall,
    ctx: &mut crate::ai::legacy::context::ContextManager,
    tx: &mpsc::Sender<AgentEvent>,
    session_log: &mut crate::ai::legacy::context::AiSessionLog,
    ...
    // Generate and emit preview for file operations (enables diff display in UI)
    if matches!(call.function.name.as_str(), "write_file" | "edit_file") {
        if !result.starts_with("Error") {
            let preview = crate::ai::legacy::agent::preview::generate_tool_preview(
                agent,
                &call.function.name,
                &call.function.arguments,
            );
...
async fn handle_approval_flow(
    ...
    let preview = crate::ai::legacy::agent::preview::generate_tool_preview(agent, &call.function.name, &call.function.arguments);
...
async fn intercept_todo_write(
            ...
            // Sync plan_steps if empty
            {
                ...
                if steps.is_empty() {
                    *steps = todos
                    ...
                    // Update existing plan steps based on todos
                    for todo in todos.iter() {
                        if let Some(step_num) = todo.id.strip_prefix("step_").and_then(|s| s.parse::<usize>().ok()) {
                            if let Some(step) = steps.get_mut(step_num - 1) {
                                step.status = match todo.status {
                                    TodoStatus::Pending => super::types::PlanStepStatus::Pending,
                                    TodoStatus::InProgress => super::types::PlanStepStatus::InProgress,
                                    TodoStatus::Completed => super::types::PlanStepStatus::Done,
                                    TodoStatus::Failed => super::types::PlanStepStatus::Failed,
    ...
    // Send updated plan steps
    let steps = agent.plan_steps.lock().await.clone();
    let _ = tx.send(AgentEvent::PlanUpdate { steps }).await;
}
